...
try {
    ...
    echo "=== KIỂM TRA BLOG ELEMENTS TRONG BẢNG FRONTENDS ===\n";
    $stmt = $pdo->prepare("
        SELECT id, data_keys, slug, status, created_at, tempname
        FROM frontends
    ...
    $stmt = $pdo->prepare("
        SELECT id, data_keys, slug, status, created_at, tempname, data_values
        FROM frontends
        WHERE id = 7
    ");
    $stmt->execute();
    ...
    // Check blog elements with correct template and status
    ...
    if ($activeBlogs) {
        echo "Có " . count($activeBlogs) . " blog elements active:\n";
        foreach ($activeBlogs as $blog) {
            echo "  - ID: {$blog['id']}, Slug: {$blog['slug']}, Template: {$blog['tempname']}\n";
        }
    } else {
        echo "❌ Không có blog elements nào active\n";
    }
