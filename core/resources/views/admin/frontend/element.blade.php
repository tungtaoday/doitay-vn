        ...
        (function($) {
            ...
            @if(@$section->element->slug)
                ...
                $('[name=slug]').on('input',function(){
                    ...
                    let slug = $(this).val();
                    let verificationDiv = closestForm.find('.slug-verification');
                    ...
                    slugCheckTimeout = setTimeout(function() {
                        $.ajax({
                            url: "{{ route('admin.frontend.sections.element.slug.check',[$key,@$data->id]) }}",
                            method: 'GET',
                            data: { slug: slug },
                            dataType: 'json',
                            headers: {
                            ...
                            error: function(jqXHR, textStatus, errorThrown) {
                                console.error('Slug check failed:', textStatus, errorThrown);
                                console.log('Response text:', jqXHR.responseText); // Debug log
                                
                                let errorMessage = 'Error checking slug';
                                let responseText = jqXHR.responseText;
                                
                                // Remove HTML comments and try to parse JSON
                                ...
                                verificationDiv.html(`
                                    <small class="text--danger"><i class="las la-times"></i> ${errorMessage}</small>
                                `);
                                submitBtn.addClass('disabled');
                            }
                        });
