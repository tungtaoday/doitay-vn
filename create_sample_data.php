...
try {
    ...
    for ($i = 1; $i <= 50; $i++) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO users (firstname, lastname, username, email, mobile, password, status, ev, sv, ts, 
    ...
    foreach ($companyIds as $index => $companyId) {
        $userId = $contractorIds[$index];
        $numRatings = rand(3, 15); // Each contractor gets 3-15 ratings
        ...
        for ($j = 0; $j < $numRatings; $j++) {
            ...
            $featureStmt = $pdo->prepare("SELECT id FROM features WHERE category_id = ?");
        ...
        $avgStmt = $pdo->prepare("
            UPDATE companies 
            SET avg_rating = (SELECT AVG(rating) FROM ratings WHERE company_id = ?),
    ...
    for ($i = 0; $i < 200; $i++) { // Create 200 appointments
        $customerId = $customerIds[array_rand($customerIds)];
        ...
        // Random date within last 3 months to next 1 month
        $dateOffset = rand(-90, 30);
    ...
    foreach ($topContractors as $contractor) {
        echo "  - {$contractor['name']} ({$contractor['category']}) - ⭐ {$contractor['avg_rating']}/5 ({$contractor['total_rating']} reviews) - {$contractor['location']}\n";
    }
    ...
    echo "\n🎉 HOÀN THÀNH! Database đã được cập nhật với dữ liệu mẫu phong phú.\n";
    echo "\nThông tin đăng nhập:\n";
    echo "- Admin: admin@example.com / password123\n";
    echo "- Customers: customerX@example.com / password123\n";  
    echo "- Contractors: thoXXX@example.com / password123\n";
