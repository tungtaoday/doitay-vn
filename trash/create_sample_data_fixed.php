...
try {
    ...
    echo "Các cột có sẵn: " . implode(', ', $columns) . "\n\n";
    ...
    for ($i = 1; $i <= 50; $i++) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO users (firstname, lastname, username, email, mobile, password, status, ev, sv, 
    ...
    foreach ($companyIds as $index => $companyId) {
        ...
        $numRatings = rand(3, 10); // Each contractor gets 3-10 ratings
    ...
    for ($i = 0; $i < 150; $i++) { // Create 150 appointments
        $customerId = $customerIds[array_rand($customerIds)];
    ...
    echo "\nThông tin đăng nhập:\n";
    echo "- Customers: [tên]@example.com / password123\n";  
    echo "- Contractors: tho[tên]@example.com / password123\n";
